...
- A full Python-compatible method set is available on the compile path (`upper`, `lower`, `strip`, `split`, `replace`, `find`, …). Case transforms in AOT (`--emit-exe`) are ASCII-oriented in the C runtime; JIT uses full Unicode case mapping.
